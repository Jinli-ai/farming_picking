...
class ImuAngleSubscriber(Node):
    def __init__(self):
        ...
        # 自定义风扇位处理顺序：1→8→2→7→3→6→4→5
        self.task2_fan_order = [1, 5, 2, 6, 3, 7, 4, 8]  # 新增：自定义顺序
        self.task2_fan_index = 0  # 新增：当前处理的顺序索引（从0开始）
        ...
        # 任务二距离参数（左侧1-4：2.3/1.8/1.3/0.8；右侧5-8：2.2/1.7/1.2/0.7）
        self.task2_left_dist = [2.3 - i*0.5 for i in range(4)]  # 1-4号距离
        self.task2_right_dist = [2 - i*0.5 for i in range(4)]  # 5-8号距离
        self.task2_dist_tol = 0.1  # 距离误差容忍度
        self.task2_yolo_detected = False  # 是否检测到果树（ap1/ap2/pe1/pe2）
    ...
    def task2_move_to_fan(self, fan_pos):
        """移动到指定风扇位的目标距离"""
        ...
        if abs(err) < self.task2_dist_tol:
            self.stop_robot()
            self.get_logger().info(f"已到达{fan_pos}号风扇位，目标距离{target_dist}m，实际{self.lidar_data:.2f}m")
            return True
        else:
            # 根据误差方向控制前进/后退
            if err > self.task2_dist_tol:
                self.back_robot1()  # 当前距离过远，后退
            elif err < -self.task2_dist_tol:
                self.move_robot1()  # 当前距离过近，前进
            return False

    def task2_switch_view(self, view_pos):
        """切换识别位置（仅保留0：上方视角，移除下方视角）"""
        self.stop_robot()
        self.vision_active = True
        self.star_yolov = True
        self.task2_yolo_detected = False
        # 强制仅使用上方视角，忽略传入的view_pos参数（防止其他逻辑误传1）
        # 上方视角（复用任务二左侧/右侧上方准备姿态）
        if 1 <= self.task2_fan_pos <= 4:
            self.task2_left_prep_on()
        else:
            self.task2_right_prep_on()
        self.get_logger().info(f"{self.task2_fan_pos}号风扇位 - 切换到上方视角（仅保留上层识别）")
        time.sleep(1.5)  # 等待机械臂到位

    def task2_check_fruit(self):
        """判断YOLO检测结果是否匹配目标果树（ap1/ap2=苹果树；pe1/pe2=梨子树）"""
        if not self.yolov_name.startswith(("ap", "pe")):
            self.get_logger().info(f"检测到非目标类型：{self.yolov_name}，仅识别ap/pe系列果树")
            return False
        target_fruit = self.task2_qr_lines[self.task2_fan_pos - 1]
        # 果树类型映射：ap系列→苹果，pe系列→梨子
        detected_type = "苹果树" if self.yolov_name.startswith("ap") else "梨子树" if self.yolov_name.startswith("pe") else "未知"
        if detected_type == target_fruit:
            self.task2_yolo_detected = True
            self.get_logger().info(f"检测到匹配果树！目标{target_fruit}，实际{self.yolov_name}")
            # 播报匹配结果
            self.play_local_wav(target_fruit)
            time.sleep(1)
            return True
        else:
            self.get_logger().info(f"检测结果不匹配！目标{target_fruit}，实际{self.yolov_name}")
            return False

    def task2_main_logic(self):
        """任务2主逻辑：二维码扫描→1-8位风扇位遍历→上下双视角识别→结果判断"""
        # 步骤1：扫描二维码（原逻辑保留，确保扫码完成后进入步骤2）
        if self.task2_step == 1:
            if not self.task2_qr_scanned:
                ...
                if not self.task2_qr_pose_adjusted:
                    ...
                    self.task2_qr_pose_adjusted = True  # 标记为已调整
            else:
                # 进入步骤2时初始化顺序索引
                self.task2_qr_pose_adjusted = False
                self.task2_step = 2
                self.task2_fan_index = 0  # 重置顺序索引为0
                self.get_logger().info("任务2进入风扇位处理阶段，按顺序1→8→2→7→3→6→4→5处理")
                return
        ...
        # 步骤2：按自定义顺序遍历处理风扇位
            ...
            # 所有风扇位处理完成（索引超出顺序列表长度），进入步骤3
            if self.task2_fan_index >= len(self.task2_fan_order):
                self.task2_step = 3
                return
            
            # 当前处理的风扇位（从自定义顺序列表中获取）
            self.task2_fan_pos = self.task2_fan_order[self.task2_fan_index]
            self.get_logger().info(f"当前处理顺序：{self.task2_fan_index+1}/8，风扇位：{self.task2_fan_pos}号")

            # 子步骤A：移动到当前风扇位
            if self.task2_sub_step != "view_check":
                self.get_logger().info(f"任务2处理{self.task2_fan_pos}号风扇位（步骤A：移动到位）")
                if not self.task2_move_to_fan(self.task2_fan_pos):
                    return  # 未到达目标距离，继续移动
                self.task2_view_pos = 0  # 先处理上方视角
                self.task2_switch_view(self.task2_view_pos)  # 切换到上方视角
                self.task2_view_timer = time.time()  # 启动视角识别计时器
                self.task2_sub_step = "view_check"  # 进入视角识别子步骤
                return

        # 子步骤B：视角识别（每个视角识别3秒，超时视为未检测）
        if self.task2_step == 2 and self.task2_sub_step == "view_check":
            current_view = "上方"  # 仅上方视角，固定文案
            # 识别超时（3秒，超时视为未检测）
            if time.time() - self.task2_view_timer > 3:
                self.get_logger().info(f"{self.task2_fan_pos}号风扇位{current_view}视角识别超时，未检测到果树")
                self.vision_active = False
                self.star_yolov = False
                # 直接标记当前风扇位完成，不切换下层视角
                self.get_logger().info(f"{self.task2_fan_pos}号风扇位上方视角识别完成，标记为完成")
                self.task2_fan_index += 1  # 处理下一个风扇位
                self.task2_sub_step = ""  # 重置子步骤
            # 检测到果树，判断是否匹配（匹配后直接标记完成）
            elif self.task2_check_fruit():
                self.vision_active = False
                self.star_yolov = False
                # 直接标记当前风扇位完成，不切换下层视角
                self.get_logger().info(f"{self.task2_fan_pos}号风扇位上方视角识别匹配，标记为完成")
                self.task2_fan_index += 1  # 处理下一个风扇位
                self.task2_sub_step = ""  # 重置子步骤
            return
    ...
    def qr_poss(self):
        ...
        servos2 = [3, 2100, 4, 1200, 9, 2150 ,8, 1500]
    ...
    def task2_left_prep_on(self):
        """任务二左侧上方准备"""
        servos1 = [1, 1075, 3, 1700, 4, 600, 9, 1700]
        self.setPWMServoMoveByArray(servos1, 4, 1100)
        time.sleep(1.1)
        
        servos2 = [3, 2145, 4, 1605, 9, 1965, 8, 2060]
        self.setPWMServoMoveByArray(servos2, 4, 1100)
        time.sleep(1.1)
    ...
    def task2_right_prep_on(self):
        """任务二右侧上方准备"""
        servos1 = [1, 2100, 3, 1700, 4, 600, 9, 1700]
        self.setPWMServoMoveByArray(servos1, 4, 1100)
        time.sleep(1.5)
    ...
    def gochang2(self):
        """调整流程：前进2.5米→右转90度→前进2米→停车→播报→静止"""
        if self.gochang2_active:
            # 步骤0：初始化（新增右转后前进2米相关参数）
            if self.gochang2_step == 0:
                self.get_logger().info("启动gochang2任务，初始化状态（含2.5米+右转后2米移动）")
                # 运动状态标记（新增“右转后2米到达”标记）
                self.reached_2_5m = False          # 是否到达第一目标：2.5米
                self.reached_2m_after_turn = False # 是否到达第二目标：右转后2米
                self.turn_step_done = False        # 转弯是否完成
                self.vehicle_stopped = False       # 小车是否最终停稳
                # 播报相关标记
                self.report_started = False
                self.report_complete = False
                # 移动参数（初始设置第一阶段目标：2.5米）
                self.target = 2.5
                self.initial_yaw_for_turn = None   # 转弯基准角度（待2.5米处设置）
                self.star_move = True              # 允许移动，先执行第一阶段前进
                self.get_logger().info("初始化完成，准备先前进到2.5米")
                self.gochang2_step = 1  # 进入第一阶段前进步骤

            # 步骤1：第一阶段前进→到达2.5米目标点
            ...
            # 步骤2：执行右转90度（基于2.5米处的角度）
                ...
                if not self.turn_step_done and self.initial_yaw_for_turn is not None:
                    ...
                    if current_error < self.angle_threshold_stop:
                        ...
                        self.stop_robot()  # 转弯后强制停车，准备第二阶段前进
                        time.sleep(1.5)
                        # 重置距离控制状态，设置第二阶段目标：2米
                        self.target = 2.0
                        self.target_finsh = False  # 清除第一阶段“完成”标记，避免影响第二阶段
                        self.get_logger().info(f"转弯完成（误差={current_error:.2f}°），准备前进2米")
                        self.gochang2_step = 3  # 进入第二阶段前进步骤

            # 新增步骤3：第二阶段前进→右转后再前进2米
            elif self.gochang2_step == 3:
                self.get_logger().info(f"gochang2步骤3/5：前往2米（当前距离：{self.lidar_data:.2f}米）")
                # 调用通用距离控制函数移动到2米
                self.go_target()
                # 到达2米后切换状态（此步骤后不再移动，进入停车播报）
                if self.target_finsh:
                    self.stop_robot()  # 核心：到达2米后强制停车
                    time.sleep(2.0)  # 延长等待时间，抵消连续移动+转弯的惯性，确保完全停稳
                    self.reached_2m_after_turn = True
                    self.vehicle_stopped = True  # 标记小车已最终停稳
                    self.get_logger().info("已到达2米目标点，小车已强制停稳")
                    self.pour()
                    self.gochang2_step = 4  # 进入播报步骤

            # 步骤4：确认停车后执行播报（适配0ge格式，含0个情况）
            elif self.gochang2_step == 4:
                # 二次确认停车（双重保险，防止意外移动）
                if not self.vehicle_stopped:
                    self.get_logger().info("二次确认停车，防止小车意外移动")
                    self.stop_robot()
                    time.sleep(1)
                ...
                if not self.report_started:
                    self.get_logger().info("gochang2步骤4/5：开始播报采收情况（适配0ge格式）")
                    self.report_started = True
                    # 果实列表（固定顺序，确保每个果实都播报，含0个）
                    fruit_count_map = [
                        ("南瓜", self.pu1_num),
                        ("辣椒", self.ch1_num),
                        ("西红柿", self.to1_num),
                    ...
                    for fruit_name, count in fruit_count_map:
                        ...
                        if fruit_name in self.audio_config["audio_paths"]:
                            ...
                            time.sleep(1.2)  # 等待名称播完（根据实际音频时长调整）
                            self.get_logger().debug(f"已播报果实名称：{fruit_name}")
                        else:
                            self.get_logger().error(f"缺失果实名称音频：{fruit_name}（路径未配置）")
                        
                        # 步骤B：播放数量（适配0ge/1ge格式，如count=0→“0ge”）
                        num_audio_key = f"{count}ge"
                        if num_audio_key in self.audio_config["audio_paths"]:
                            self.play_local_wav(num_audio_key)
                            time.sleep(1)  # 等待数量播完
                            self.get_logger().debug(f"已播报数量：{num_audio_key}（实际计数：{count}个）")
                        else:
                            self.get_logger().error(f"缺失数量音频：{num_audio_key}（路径未配置）")
                    
                    self.report_complete = True
                    self.get_logger().info("播报完成（含0个情况）")
                    self.gochang2_step = 5  # 进入最终终止步骤

            # 步骤5：彻底终止任务，锁定静止状态
            elif self.gochang2_step == 5:
                self.get_logger().info("gochang2步骤5/5：彻底终止任务，锁定静止状态")
                # 三重保险：强制停止所有动作（防止任何残留运动）
                self.stop_robot()
                self.stop_audio()  # 停止可能残留的音频进程（避免一直播放）
                # 禁用所有运动相关标记（防止其他逻辑误触发移动）
                self.star_move = False
                self.is_turning = False
                self.target_finsh = True
                # 彻底关闭任务（防止再次进入gochang2逻辑）
                self.gochang2_active = False
                self.gochang2_step = 0
                self.get_logger().info("全流程完成：2.5米→右转→2米→停车→播报，小车保持静止")
    def gage_name(self):
    ...
    def grad_adiust(self):
        """调整机械臂对准目标"""
        if self.trage_fand:
            # 修复：添加机械臂姿态有效性判断，避免方向反向
            if (self.arm_left and not self.arm_right) or (self.arm_right and not self.arm_left):
                if self.arm_left:
                ...
                # 机械臂姿态未就绪，输出警告并停止调整
                self.get_logger().warn("机械臂姿态未就绪（arm_left/arm_right未正确设置），停止目标对准")
                self.trage_fand = False
