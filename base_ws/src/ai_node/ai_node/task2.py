...
class ImuAngleSubscriber(Node):
    def __init__(self):
        ...
        self.task2_recognition_timeout = 3.0  # 单视角识别超时
    ...
    def task2_main_logic(self):
        ...
        if self.task2_step == 3:
            ...
            self.get_logger().info("任务2所有树处理完成！")
            self.play_local_wav("完成")
            time.sleep(2)
            self.task_on2 = False
            self.gochang1 = True  # 完成后进入交换区
            self.star_move = False
    # -------------------------- 原有方法保留（新增任务2调用） --------------------------
    # 音频控制方法
    def stop_audio(self):
    ...
    def qr_poss(self):
        """调整摄像头姿态读取QR码"""
        servos1 = [1, 2075, 3, 1700, 4, 600, 9, 2100]
    ...
    def task2_left_prep_on(self):
        ...
        servos2 = [3, 2070, 4, 1925, 9, 1565, 8, 2060]
        self.setPWMServoMoveByArray(servos2, 4, 1100)
        time.sleep(1.1)
    ...
    def task2_on_grad(self):
        ...
        servos = [3, 1460, 4, 1415, 9, 1000, 8, 1600]
        self.setPWMServoMoveByArray(servos, 4, 3000)
        time.sleep(3.5)
    ...
    def task2_under_grad(self):
        """下侧抓取动作"""
        servos = [3, 1200, 4, 1620, 9, 750, 8, 1800]
        self.setPWMServoMoveByArray(servos, 4, 3000)
        time.sleep(3.5)
        ...
        servos = [3,1800 , 4 , 1200 ,9 ,2100 ,8 ,2100]
    ...
    def gochang2(self):
        if self.gochang2_active:
            # 步骤0：初始化（含2.5米前进+右转90度+前进2米+左转90度完整流程）
            if self.gochang2_step == 0:
                self.get_logger().info("启动gochang2任务，初始化状态（完整路径：2.5米→右转→2米→左转）")
                # 运动状态标记
                self.reached_2_5m = False          # 是否到达第一目标：2.5米
                self.reached_2m_after_turn = False # 是否到达第二目标：右转后2米
                self.turn_step_done = False        # 转弯是否完成（复用，先右转后左转）
                ...
                self.target = 2.5
                self.initial_yaw_for_turn = None   # 转弯基准角度（2.5米处设置）
            ...
            # 步骤2：执行右转90度（基于2.5米处的角度）
                ...
                if not self.turn_step_done and self.initial_yaw_for_turn is not None:
                    # 计算相对目标角度：2.5米处角度 + 90度（相对角度避免绝对误差）
                    ...
                    if not self.is_turning:
                        ...
                        self.get_logger().info(
                            f"开始右转：基准角度={self.initial_yaw_for_turn:.2f}°，目标={target_absolute_yaw:.2f}°"
                        )
                    ...
                    if current_error < self.angle_threshold_stop:
                        ...
                        self.target = 2.0
                        self.target_finsh = False  # 清除第一阶段“完成”标记
                        self.get_logger().info(f"右转完成（误差={current_error:.2f}°），准备前进2米")
                ...
                if not self.report_started:
                    ...
                    for fruit_name, count in fruit_count_map:
                        ...
                        if fruit_name in self.audio_config["audio_paths"]:
                            ...
                            time.sleep(1.2)  # 等待名称播完
                            self.get_logger().debug(f"已播报果实名称：{fruit_name}")
                        else:
                            self.get_logger().error(f"缺失果实名称音频：{fruit_name}")
                        
                            # 步骤B：播放数量（适配0ge/1ge格式）
                            ...
                            if num_audio_key in self.audio_config["audio_paths"]:
                                ...
                                self.get_logger().error(f"缺失数量音频：{num_audio_key}")
                        
                        self.report_complete = True
                        self.get_logger().info("播报完成（含0个情况）")
                        # 播报完成后，重置转弯状态，准备左转
                        self.turn_step_done = False  # 重置转弯标记，用于左转判断
                        self.star_move = True
                        self.gochang2_step = 5  # 进入左转步骤

                # 步骤5：执行左转90度（回到初始方向，修复核心逻辑）
                elif self.gochang2_step == 5:
                    self.get_logger().info("gochang2步骤5/5：执行左转90度")
                    if not self.turn_step_done and self.initial_yaw_for_turn is not None:
                        # 计算相对目标角度：2.5米处初始角度 - 90度（回到原方向）
                        target_absolute_yaw = self.normalize_angle(self.initial_yaw_for_turn - 90.0)
                        # 启动转弯
                        if not self.is_turning:
                            self.goal_yaw = target_absolute_yaw
                            ...
                            self.turn_target_reached = False
                            self.get_logger().info(
                                f"开始左转：基准角度={self.initial_yaw_for_turn:.2f}°，目标={target_absolute_yaw:.2f}°"
                            )
                        
                        # 左转完成判断（关键：必须等待角度误差小于阈值）
                        current_error = abs(self.normalize_angle(target_absolute_yaw - self.yaw_deg))
                        if current_error < self.angle_threshold_stop:
                            self.is_turning = False
                            self.turn_target_reached = True
                            self.turn_step_done = True
                            self.stop_robot()  # 左转后强制停车
                            time.sleep(1.5)
                            self.get_logger().info(f"左转完成（误差={current_error:.2f}°）")
                            ...
                            self.target_finsh = False
                            self.star_move = True
                            self.gochang2_step = 0  # 重置步骤，避免下次异常触发
                            self.get_logger().info("gochang2流程全部完成，准备运行后续任务")
    def gage_name(self):
        ...
        if self.trage_fand:
            ...
            if self.task_on2 and self.task2_step == 2:
                if 310.0 <= self.yolov_x <= 330.0:
                    self.in_mid = True
                    self.stop_robot()
                    self.get_logger().info(f"目标居中完成（X={self.yolov_x:.1f}）")
                else:
                    self.in_mid = False
                    speed = self.align_speed
                    if self.yolov_x < 300.0:
                        self.twist.linear.x = -speed
                    elif self.yolov_x > 340.0:
                        self.twist.linear.x = speed
                    self.vel_publisher.publish(self.twist)
            else:
                # 其他任务保持原有逻辑
